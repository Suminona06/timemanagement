...
import { Calendar, Clock, CheckCircle2, ChevronRight, CheckSquare, Sparkles } from 'lucide-react';
    ...
    <div className="card p-5 bg-surface-800 border-surface-700 space-y-4 flex flex-col justify-between h-full">
      ...
      {!hasItems ? (
        <div className="flex flex-col items-center justify-center py-8 text-surface-500 gap-2">
          ...
          <p className="text-xs">No scheduled tasks or logs for today.</p>
        </div>
      ) : (
        <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
          ...
          {todayTasks.length > 0 && (
            <div className="space-y-1.5">
              ...
              {todayTasks.map((t) => (
                <div
                  key={t._id}
                  className="flex items-center justify-between p-2 rounded-lg bg-surface-850 border border-surface-700/80 text-xs"
                >
                  <div className="flex items-center gap-2 truncate min-w-0 pr-2">
                    <CheckCircle2
                      size={14}
                      className={t.status === 'Completed' ? 'text-success-400' : 'text-surface-500'}
                    />
                    <span
          ...
          {todayLogs.length > 0 && (
            <div className="space-y-1.5 pt-1">
              <span className="text-[11px] font-semibold text-surface-400 uppercase tracking-wider block">
                Completed Sessions ({todayLogs.length})
              </span>
              {todayLogs.map((log) => (
                <div
                  key={log._id}
                  className="flex items-center justify-between p-2 rounded-lg bg-primary-500/5 border border-primary-500/20 text-xs"
                >
                  <div className="flex items-center gap-2 truncate min-w-0 pr-2">
                  ...
                  <div className="flex items-center gap-1.5 text-[11px] text-surface-400 font-mono shrink-0">
                    <Clock size={11} />
