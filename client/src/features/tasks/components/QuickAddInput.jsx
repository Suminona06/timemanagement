    ...
    <form onSubmit={handleSubmit} className={`flex items-center gap-2 ${className}`}>
      <div className="relative flex-1">
        <input
          ...
          className="input pl-3.5 pr-8 py-2 text-xs sm:text-sm shadow-warm-sm"
      ...
      {categories.length > 0 && (
        <select
          ...
          disabled={isSubmitting}
          className="input py-2 px-3 text-xs w-auto max-w-[130px] pr-7 text-surface-700 dark:text-surface-300 cursor-pointer rounded-xl shadow-warm-sm"
        >
      ...
      <button
        ...
        className="btn-primary py-2 px-3.5 text-xs sm:text-sm shrink-0 shadow-warm-sm"
        title="Add task"
        aria-label="Add task"
      >
        <Plus size={15} />
