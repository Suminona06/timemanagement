...
export default function EstimationAccuracyCard({ data = {} }) {
  ...
  const getScoreColor = (score) => {
    if (score >= 85) return 'text-pastel-matcha-dark dark:text-pastel-matcha bg-pastel-matcha-light/40 dark:bg-pastel-matcha/20 border-pastel-matcha/30';
    if (score >= 65) return 'text-pastel-chai-dark dark:text-pastel-chai bg-pastel-chai-light/40 dark:bg-pastel-chai/20 border-pastel-chai/30';
    return 'text-pastel-peach-dark dark:text-pastel-peach bg-pastel-peach-light/40 dark:bg-pastel-peach/20 border-pastel-peach/30';
  };
  ...
  return (
    <div className="card p-5 bg-surface-50 dark:bg-surface-800 border-surface-300 dark:border-surface-700/80 shadow-warm-sm space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-surface-200 dark:border-surface-700/60">
        ...
        <span
          className={`text-[11px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full border ${getScoreColor(
            scorePercentage
          )}`}
        >
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="space-y-0.5">
          <div className="text-3xl sm:text-4xl font-extrabold text-surface-900 dark:text-surface-100 font-mono tracking-tight">
            {scorePercentage}%
          </div>
        ...
        <div className="space-y-1.5 text-xs text-right font-medium">
          <div className="flex items-center justify-end gap-1.5 text-pastel-matcha-dark dark:text-pastel-matcha">
            <CheckCircle2 size={13} />
            <span>{accurateTasksCount} on target (±15%)</span>
          </div>
          <div className="flex items-center justify-end gap-1.5 text-pastel-chai-dark dark:text-pastel-chai">
            <TrendingUp size={13} />
            <span>{underEstimatedCount} took longer</span>
          </div>
          <div className="flex items-center justify-end gap-1.5 text-surface-500">
