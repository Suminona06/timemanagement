  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-surface-850 p-2.5 rounded-xl border border-surface-700 shadow-xl text-xs space-y-1">
        <div className="flex items-center gap-2 font-semibold text-surface-100">
          <span
        ...
        <div className="text-surface-300">
          Duration: <strong className="text-surface-100">{formatMinutes(data.totalMinutes)}</strong> ({data.totalHours} hrs)
        </div>
        <div className="text-surface-400">
          Share: <strong className="text-primary-400">{data.percentage}%</strong> ({data.sessionCount} sessions)
        </div>
      </div>
    );
  }
    ...
    <div className="card p-5 bg-surface-800 border-surface-700 flex flex-col justify-between h-full">
      ...
      {!hasData ? (
        <div className="flex flex-col items-center justify-center py-12 text-surface-500 gap-2">
          <PieIcon size={32} className="opacity-30" />
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 pt-4">
          ...
          <div className="w-full md:w-1/2 space-y-2 max-h-48 overflow-y-auto pr-1">
            {data.map((cat) => (
              <div
                key={cat.name}
                className="flex items-center justify-between p-1.5 rounded-lg hover:bg-surface-750 transition-colors text-xs"
              >
                ...
                <div className="flex items-center gap-2 shrink-0 font-mono">
                  <span className="text-surface-400">{formatMinutes(cat.totalMinutes)}</span>
                  <span className="font-semibold text-surface-200 w-8 text-right">
                    {cat.percentage}%
                  </span>
                </div>
              </div>
